#!/usr/bin/env python3
"""
Example usage of the Straico API Python client.
...
def main():
    ...
    print("-" * 60)
    response = client.chat("Explain quantum entanglement in simple terms", pricing_method="quality")
    if response.get("success"):
        data = response["data"]
        choices = data["completion"]["choices"]
    ...
    print("\n3. Budget query:")
    print("-" * 60)
    response = client.chat("What is 2+2?", pricing_method="budget")
    if response.get("success"):
        data = response["data"]
        choices = data["completion"]["choices"]
        print(f"Response: {choices[0]['message']['content']}")
    ...
    print("-" * 60)
    response = client.chat("Tell me a short joke", model="openai/gpt-4o-mini")
    if response.get("success"):
        data = response["data"]
        choices = data["completion"]["choices"]
        print(f"Response: {choices[0]['message']['content']}")
    ...
    response = client.chat(
        "What is AI?", models=["openai/gpt-4o-mini", "anthropic/claude-3-5-haiku-20241022"]
    )
    if response.get("success"):
        data = response["data"]
    ...
    print("-" * 60)
    response = client.chat(
        "Compare different perspectives on renewable energy", pricing_method="balance", quantity=2
    )
    if response.get("success"):
        data = response["data"]
    ...
    print("-" * 60)
    response = client.chat("Hello", model="gpt4")  # Invalid model name
